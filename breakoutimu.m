...
global paddle bricks ball paddlePos X Y State Sound Speed ...
    hScore hSplash hSound hSpeed Pitch_value  data_Pitch_Left data_Pitch_Right flt_data_1 flt_data_3 flt_data_2 avg_data_1 avg_data_2 avg_data_3
...
switch cmd
    ...
    case 'NewGame'
        ...
        paddlePos = [X/4,0,X/6,Y/20];
        while(State)
            % 왼쪽으로 최대로 회전했을 때 X=0 / 오른쪽으로 최대로 회전했을 때의 X = 350으로 설정
            
             cp = avg_data_3; % Moving Average Filter를 거친 값
             
             % 1차 선형식이라고 생각하고 알고있는 두 점으로 구한 1차식
            ...
            elseif paddlePos(1)>X-(X/6)   
                ...
                paddlePos(1) = (385/(data_Pitch_Right-data_Pitch_Left))*cp -((385/(data_Pitch_Right-data_Pitch_Left))*data_Pitch_Left) ;
